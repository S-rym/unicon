...
options = Options()
#options.add_argument("--headless")  # ヘッドレスモードで実行（画面表示なし）
options.add_argument("--no-sandbox")  # セキュリティ関連のオプション（root環境の場合必要）
options.add_argument("--disable-dev-shm-usage")  # 共有メモリ不足回避用
